...
@app.post("/users/",response_model=List[page_schemas.User])
def create_user(entrada:page_schemas.User,db:session=Depends(get_user)):
    usuario = page_models.User(username = entrada.username,contrasena = entrada.contrasena, email = entrada.email, estado = entrada.estado)
    db.add(usuario)
    db.commit()
    db.refresh(usuario)
    return usuario

@app.put("/users/{usuario_id}",response_model=List[page_schemas.User])
...
@app.delete("/users/{usuario_id}",response_model=List[page_schemas.User])
