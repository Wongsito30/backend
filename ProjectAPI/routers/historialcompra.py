...
@router.get("/his/", response_model=List[page_schemas.Historial])
async def show_his(db:session=Depends(get_his)):
    historial = db.query(page_models.Historial).all()
    return historial

@router.get("/searchhis/{hisname}", response_model=List[page_schemas.Historial])
async def show_his_user(hisname: str, db: session = Depends(get_his)):
    # Filtra los productos que coinciden con el nombre
    historial = db.query(page_models.Historial).filter(page_models.Historial.nickname == hisname).all()
...
@router.post("/his/",response_model=page_schemas.Historial)
...
@router.put("/his/{his_id}",response_model=page_schemas.Historial)
...
@router.delete("/his/{his_id}",response_model=page_schemas.respuesta)
