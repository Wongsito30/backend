...
@router.get("/wish/", response_model=List[page_schemas.Wishlist])
async def show_wish(db:session=Depends(get_wish)):
    wishlist = db.query(page_models.Wishlist).all()
    return wishlist

@router.get("/searchfav/{favname}", response_model=List[page_schemas.Wishlist])
...
@router.post("/wish/",response_model=page_schemas.Wishlist)
...
@router.put("/wish/{wish_id}",response_model=page_schemas.Wishlist)
...
@router.delete("/wish/{wish_id}",response_model=page_schemas.respuesta)
